...
$(function() {
    $("#button1").click(function() {
    	...
    	var dotPosition2 = $("#file2").val().lastIndexOf(".");
    	if (dotPosition == -1 || dotPosition == 0 || dotPosition2 == -1 || dotPosition2 == 0) {
    		alert("올바른 첨부파일이 아닙니다.");
            $("#file1").focus();
            return false;
        }
    	var fileExtension = $("#file1").val().slice(dotPosition + 1).toLowerCase();
    	var fileExtension2 = $("#file2").val().slice(dotPosition2 + 1).toLowerCase();
        ...
        if ($("#file2").val() == "") {
            ...
            $("#file2").focus();
            return false;
        }else if(fileExtension2!="mp4"&&fileExtension2!="avi"){
        	alert("영화예고편은 동영상파일이여야합니다.");
            $("#file2").focus();
            return false;
        }
        ...
        $.ajax({
            type: "POST",
            data: form,
            url: "/adminMovieInfoWriteSave.do",
...
<body>
	...
	<form name="frm" id="frm">
		...
		<form  id="frm" enctype="multipart/form-data">
			<table class="write1">
				<caption><h2>영화등록(담당자 ver)</h2></caption>
				<tr>
				<th width="15%">영화제목</th>
				<td><input type="text" name="mvtitle" id="mvtitle" placeholder="제목을 입력하세요"></td>
			</tr>
			<tr>
				<th width="5%">영화등급</th>
				<td>
			...
			<tr>
				...
				<td><input type="file" name="file1" id="file1" accept="image/jpg,image/jpeg,image/png"><input type="hidden" name="mvpost" id="mvpost"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="date" name="lastday" id="lastday"></td>
			</tr>
			<tr>
				<th width="15%">영화감독</th>
				<td><input type="text" name="mvdirector" id="mvdirector" placeholder="감독명을 입력하세요"></td>
			</tr>
			<tr>
				<th width="15%">영화장르</th>
				<td><input type="text" name="mvgenre" id="mvgenre" placeholder="장르를 입력하세요"></td>
			</tr>
			<tr>
				<th width="25%">영화줄거리</th>
				<td><input type="text" name="mvinfo" id="mvinfo" placeholder="줄거리를 입력하세요"></td>
			</tr>
			<tr>
				<th width="10%">영화예고편</th>
				<td><input type="file" name="file2" id="file2" accept="video/mp4,video/avi"><input type="hidden" name="mvprev" id="mvprev"></td>
			</tr>
			<tr>
				<th width="10%">런닝타임</th>
				<td><input type="text" name="mvvolume" id="mvvolume" placeholder="ex) 2:30"></td>
			</tr>
			</table>
